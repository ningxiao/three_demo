<template>
    <section class="table">
        ...
        <main class="table-box">
            <div
                class="table-box-y-space"
                :style='"height:" + ySpaceHeight + "px"'
            ></div>
            <table
                border="0"
                cellspacing="0"
            ...
            >
                ...
                <tbody>
                    <tr
                        class="table-main-row"
                        v-for="data in viewableTableData"
                        :key="data.id"
                    >
                        <th
                            v-for="(vo,index) in theads"
...
export default class App extends Vue {
    ...
    theads: Array<{ width: string, name: string, key: string }> = [
        {
            width: '200px',
            name: '#',
            key: 'id'
    ...
    calculateLayout() {
        const size = this.theads.length - 1;
        const clientRect = this.tableBox.getBoundingClientRect();
        const vWidth = this.mainWidth - Number.parseInt(this.theads[0].width);
        const maxWidth = Math.floor(vWidth / size);
        this.mainWidth = clientRect.width;
        this.mainHeight = clientRect.height;
        this.maxTop = this.ySpaceHeight - this.mainHeight;
        for (let i = 1; i < this.theads.length; i++) {
            this.theads[i].width = `${maxWidth}px`;
        }
    }
    calculateScroll() {
        let scrollTop = document.body.querySelector('.table-box-y-space').getBoundingClientRect().top;
        if (scrollTop >= 0) {
            scrollTop = 49 - scrollTop;
        } else {
            scrollTop = 49 + Math.abs(scrollTop);
        }
        const { height } = this.tableMain.getBoundingClientRect();
        if (this.baseScrollTop < scrollTop) {
            if (this.mainHeight >= height + this.tableMainTop - scrollTop) {
                console.log(this.dataStart, this.dataEnd);
                this.dataStart = this.dataStart + Math.ceil((height - this.mainHeight) / this.rowHeight) - 1;
                this.dataEnd = Math.min(this.tableData.length, this.dataStart + Math.ceil(this.mainHeight / this.rowHeight) + 4);
                console.log(this.dataStart, this.dataEnd);
                this.viewableTableData = this.tableData.slice(this.dataStart, this.dataEnd);
                this.tableMainTop = Math.min(scrollTop - 33, this.maxTop);
            }
        }
        this.tableMain.style.transform = `translate3d(0px, ${this.tableMainTop}px, 0px)`;
        this.baseScrollTop = scrollTop;
        // const { height } = this.tableMain.getBoundingClientRect();
        // if (this.baseScrollTop < scrollTop) {
        //     if (this.mainHeight >= height + this.tableMainTop - scrollTop) {
        //         console.log(this.dataStart, this.dataEnd);
        //         this.dataStart = this.dataStart + Math.ceil((height - this.mainHeight) / this.rowHeight);
        //         this.dataEnd = Math.min(this.tableData.length, this.dataStart + Math.ceil(this.mainHeight / this.rowHeight) + 4);
        //         console.log(this.dataStart, this.dataEnd);
        //         this.viewableTableData = this.tableData.slice(this.dataStart, this.dataEnd);
        //         this.tableMainTop = Math.min(scrollTop, this.maxTop);
        //     }
        // }
        // this.tableMain.style.transform = `translate3d(0px, ${this.tableMainTop}px, 0px)`;
        // const scrollTop = this.tableBox.scrollTop;
        // if (scrollTop > this.minTop && scrollTop < this.maxTop) {
        //     const { height } = this.tableMain.getBoundingClientRect();
        //     if (this.baseScrollTop < scrollTop) {
        //         if (this.mainHeight >= height + this.tableMainTop - scrollTop) {
        //             console.log(this.dataStart, this.dataEnd);
        //             this.dataStart = this.dataStart + Math.ceil((height - this.mainHeight) / this.rowHeight);
        //             this.dataEnd = Math.min(this.tableData.length, this.dataStart + Math.ceil(this.mainHeight / this.rowHeight));
        //             console.log(this.dataStart, this.dataEnd);
        //             this.viewableTableData = this.tableData.slice(this.dataStart, this.dataEnd);
        //             this.tableMainTop = Math.min(scrollTop, this.maxTop);
        //         }
        //     } else {
        //         if (this.tableMainTop >= scrollTop) {
        //             console.log(this.dataStart, this.dataEnd);
        //             this.dataStart = Math.max(0, this.dataStart - Math.ceil((height - this.mainHeight) / this.rowHeight));
        //             this.dataEnd = this.dataStart + Math.ceil(this.mainHeight / this.rowHeight);
        //             console.log(this.dataStart, this.dataEnd);
        //             this.viewableTableData = this.tableData.slice(this.dataStart, this.dataEnd);
        //             this.tableMainTop = Math.max(scrollTop - (height - this.mainHeight), this.minTop);
        //         }
        //     }
        //     this.tableMain.style.transform = `translate3d(0px, ${this.tableMainTop}px, 0px)`;
        //     this.baseScrollTop = scrollTop;
    ...
    mounted() {
        this.tableBox = document.body.querySelector('.table-box');
        this.tableMain = document.body.querySelector('.table-main');
        this.tableBox.addEventListener("scroll", this.calculateScroll.bind(this));
        this.domObserver = new ResizeObserver(this.calculateLayout.bind(this));
        this.domObserver.observe(this.tableBox);
        this.ySpaceHeight = this.tableData.length * this.rowHeight;
        window.requestAnimationFrame(() => {
            this.calculateLayout();
            this.dataEnd = this.dataStart + Math.ceil(this.mainHeight / this.rowHeight) + 4;
            this.viewableTableData = this.tableData.slice(this.dataStart, this.dataEnd);
            // this.viewableTableData = this.tableData;
            window.requestAnimationFrame(() => {
                this.rowDoms = this.tableMain.querySelectorAll('.table-main-row');
                this.rowHeight = this.rowDoms[0].getBoundingClientRect().height;
                this.ySpaceHeight = this.tableData.length * this.rowHeight;
            });
        });
    }
